...
class FakeFileSystem : IFileSystem
{
    ...
    {
        Console.WriteLine("====> {0}: {1}", MethodBase.GetCurrentMethod().Name, path);
        FileSystemEntry entry = CreateEntry(path);
        List<KeyValuePair<string, ulong>> result = new();
    ...
    {
        Console.WriteLine("====> {0}: {1}", MethodBase.GetCurrentMethod().Name, path);
        List<FileSystemEntry> ents = new()
        {
            CreateEntry(Path.Combine(path, "file.bin"))
        };
        return ents;
    }
    ...
    {
        return new FileSystemEntry(path, Path.GetFileName(path), path.EndsWith("\\"), 1024 * 1024 * 1024,
